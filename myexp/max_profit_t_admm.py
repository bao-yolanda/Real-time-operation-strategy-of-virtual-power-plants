...
from myexp.max_profit_1_admm import (
    AdmmSettings,
    _compute_tolerances,
    _diag_entries,
...
def max_profit_t_admm(ctx: Dict[str, Any]) -> None:
    ...
    print(
        f"slot {cur_slot}: start rolling ADMM, max_iter={settings.max_iter}, "
        f"rest_slots={rest_slots}, EV_rows={num_ev_rows}",
        flush=True,
    )
    for iter_idx in range(1, settings.max_iter + 1):
        ...
        if should_report:
            print(
                f"slot {cur_slot}: rolling ADMM iter "
    ...
    if ok:
        ...
        result["Bid_R_rev"][cur_slot_idx + 1 :] = r_der_future_full.sum(axis=0)
